...
| Resource project, character, tag, loading, and error state; workspace composition | `useProjectState.ts`, `useCharacterWorkspace.ts`, `useProjectTags.ts`, composed by `useProjectWorkspace.ts` | #34/#35/#36 establish resource-level ownership; #39 verifies the final composition                                                         |
...
| HTTP transport adapters                                                           | `src/features/character-profiles/api/`                                                                      | Feature API modules own resource paths, transport, and payloads; transport remains feature-owned until concrete cross-feature reuse exists |
